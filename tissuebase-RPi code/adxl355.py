...
class ADXL355:
    ...
    def spi_write_reg(self, address, value):
        ...
        self.spi.xfer2([(address<<1), value])
    ...
    def spi_read_reg(self, address):
        ...
        response = self.spi.xfer2([(address<<1)|0x01, 0x00])
        return response[-1]
    ...
    def read_FIFO(self,count):
        ...
        tx_buf=[0x00]*(count+1)
        tx_buf[0]=(0x11<<1)|0x01
        value = self.spi.xfer2(tx_buf)
        return value[1:]
        
    def FIFO_cnt(self):
        
        response = self.spi.xfer2([(0x05<<1)|0x01, 0x00])
        return response[-1]

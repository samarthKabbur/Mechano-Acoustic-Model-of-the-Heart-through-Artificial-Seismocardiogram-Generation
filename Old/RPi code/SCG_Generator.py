...
def calibrate_beat(beat,beat_sample_rate,transfer_funtion,transfer_funtion_freq,plot=False):
    ...
    beat_fft = (fft(beat,beat.shape[-1])) # fft of beat with the axis as the last dimension of the beat array
    beat_fft_freq = (fftfreq(beat.shape[-1],d=1/(beat_sample_rate))) # find sample frequencies of beat

    controller = np.ones(len(beat_fft)) ## length of fft is = or len(original_sig)/2 + 1
...
class bcolors:
    # colors to print error messages
    OK = '\033[92m' #GREEN
...
# The GPIO.setmode(GPIO.BOARD) line sets the numbering mode for the GPIO pins on the Raspberry Pi to
# use the physical pin numbers on the board. This is necessary to correctly identify and control the GPIO pins.
...
## making sure channel 0 is producing the sweep
    ...
    channel0.set_volume(0.0,0.2) ## to increase SCG amplitude
...
def generate_process(z_axis,startRec,doneRec,fileName,output):
    ...
    channel1=mixer.Channel(1)
    channel1.set_volume(0.8, 0.0)  ## to increase ECG amplitude
